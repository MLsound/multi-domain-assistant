...
and computes per-question metrics:
  ...
  - context_precision  (Ragas 0.2 — LLM-based, async)          [NEW]
  - answer_relevancy   (Ragas 0.2 — LLM-based, async)          [NEW]
  ...
  - rouge_l
  - tool_*_success     (4 deterministic per-agent flags)        [NEW]
...
Metric results logged to MLflow automatically.
"""
...
# Sentinel value used when a metric cannot be computed
...
def _baseline_summary(summary: Dict[str, Any]) -> list[str]:
    """
    Compare current evaluation summary against previously persisted baselines
    and produce human-readable tuning recommendations.

    Baselines are stored in ``reports/eval_baseline.json``.
    """
    ...
    # Thresholds for flagging a regression
    DROPS = {
...
async def _score_new_metrics_async(
    ...
    """
    Concurrently compute context_precision and answer_relevancy for all questions.

    ``per_question_data`` is a list of dicts with keys:
        user_input, reference, retrieved_contexts, response

    Returns the same list with cp_score and ar_score added.
    """
...
def run_evaluation(
    ...
    with mlflow_manager.start_run(run_name=f"eval-{int(time.time())}") as run:
        ...
        # P1: Compute deterministic tool-call metrics
        ...
        per_question_metrics.append({
            ...
            "sources_cited": result.get("sources_cited", []),
            "retry_count": result.get("retry_count", 0),
            # Store context texts for later async metric scoring
            "retrieved_context_texts": context_texts,
            # P1: New metric fields (placeholder until async scoring fills them)
            "context_precision": None,
            "answer_relevancy": None,
            # P1: Deterministic tool-call metrics
            **tool_flags,
        })
    ...
    # --- Ragas 0.2 evaluation (faithfulness + context_recall) ---
    eval_dataset = EvaluationDataset(samples=ragas_samples)
    run_cfg = RunConfig(max_workers=1, timeout=300)
    ...
    try:
        ...
        logger.exception("Ragas evaluation failed — continuing with non-LLM metrics only")
    ...
    # --- New: Async RAGAS metrics (context_precision, answer_relevancy) ---
    ...
    try:
        scored = asyncio.run(
            _score_new_metrics_async(
                evaluator_langchain_llm,
                async_data,
                max_concurrent=5,
            )
        )
        ...
        logger.exception("Async metric scoring failed — continuing without context_precision/answer_relevancy")
    ...
    try:
        with mlflow_manager.start_run(
            ...
            tags={
                "provider": registry.provider_name,
                "n_questions": str(len(per_question_metrics)),
                "type": "evaluation",
            },
        ):
            # Log parameters
            ...
            # Log per-question metrics as artifact
            out_artifact = Path("reports/eval_mlflow_artifact.json")
            out_artifact.parent.mkdir(parents=True, exist_ok=True)
            with open(out_artifact, "w", encoding="utf-8") as f:
                json.dump(per_question_metrics, f, indent=2, ensure_ascii=False)
            mlflow_manager.log_artifact(str(out_artifact))
    except Exception:
        logger.exception("MLflow logging failed — continuing without it")

    # --- Baseline + tuning recommendations ---
    ...
    try:
        ...
        logger.info("Baseline persisted to %s", baseline_path)
    except Exception:
        logger.exception("Could not persist baseline")
